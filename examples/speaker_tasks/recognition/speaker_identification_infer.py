...
def main(cfg):
    ...
    if backend == 'cosine_similarity':
        ...
        matched_labels_and_scores = [[(label, score) for label, score in enumerate(row)] for row in scores]

        matched_labels = [[label for label, score in row] for row in matched_labels_and_scores]
        matched_scores = [[score for label, score in row] for row in matched_labels_and_scores]
        ...
        print(test_logits)
        matched_labels_and_scores = [[(label, score) for label, score in enumerate(row)] for row in test_logits]

        print(matched_labels_and_scores)
        matched_labels = [[label for label, score in row] for row in matched_labels_and_scores]
        matched_scores = [[score for label, score in row] for row in matched_labels_and_scores]
    ...
    with open(test_manifest, 'rb') as f1, open(out_manifest, 'w', encoding='utf-8') as f2:
        ...
        for idx, line in enumerate(lines):
            ...
            matched_labels_for_item = matched_labels[idx]
            matched_scores_for_item = matched_scores[idx]
            item['infer'] = [(enroll_id2label[label], float(score)) for label, score in zip(matched_labels_for_item, matched_scores_for_item)]
