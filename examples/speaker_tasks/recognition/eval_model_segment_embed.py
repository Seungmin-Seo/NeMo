...
def get_acc(trial_file='', emb='', save_kaldi_emb=False):
    ...
    with open(trial_file, 'r') as f:
        ...
        for line in tqdm(tmp_file):
            line = line.strip()
            truth, model, segment = line.split()
            model = model.split('/')
            model = '@'.join(model)
            ...
            model_emb = emb[model]  # Assuming you have precomputed model embeddings
            segment_emb = emb[segment]

            if save_kaldi_emb and model not in keys:
                keys.append(model)
                trial_embs.extend([model_emb])

            if save_kaldi_emb and segment not in keys:
                keys.append(segment)
                trial_embs.extend([segment_emb])

            score = np.dot(model_emb, segment_emb) / ((np.dot(model_emb, model_emb) * np.dot(segment_emb, segment_emb)) ** 0.5)
            score = (score + 1) / 2
